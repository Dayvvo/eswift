import { Box, Checkbox, Flex, FormControl, FormHelperText, FormLabel, Input, InputGroup, InputLeftAddon, InputLeftElement, InputRightElement, Stack, Text } from "@chakra-ui/react"
...
export const SignUpcreen =()=> {
    ...
    const handleSubmit = (event:ChangeEvent<HTMLFormElement>) => {
        ...
        axios.post('/api/auth/signup',isSignup)
            .then(
                (res)=> {
    ...
    const HandleGoogleSubmit = async() => {
        ...
        window.addEventListener('message', async (event) => {
            ...
            if (event.data && event.data.type === 'googleAuthCode' && event.data.requestId === requestId ) {
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        >
                            ...
                            <FormControl w={'100%'}>
                                ...
                                {   emailValidation ? <></> :
                                    <FormHelperText color={'var(--errorBase)'} fontSize={'12px'}>{ isSignup? "Enter a valid email address*" : "" }</FormHelperText>
                                }
                            </FormControl>
                            <FormControl w={'100%'}>
                                <FormLabel
                                ...
                                <InputGroup 
                                    ...
                                    _placeholder={{textColor:'var--(soft400)', fontSize:{base:8,lg:12}}}
                                >
                                    <InputLeftAddon pointerEvents='none'bg={'transparent'} color={'var(--soft400)'} width={'fit-content'} height={'100%'}>
                                        +234
                                    </InputLeftAddon>
                                    <Input 
                                        w={'100%'} h={'100%'}
                            ...
                            <FormControl w={'100%'}>
                                ...
                                >
                                    ...
                                    {   passWordValidation ? <></> :
                                        <FormHelperText color={'var(--errorBase)'} fontSize={'12px'}>{ isSignup?.password.length < 8 ? "minimum of 8 characters*" : "" }</FormHelperText>
                                    }
                            </FormControl>
                            <FormControl w={'100%'}>
                                <FormLabel
                                ...
                                >
                                    ...
                                    {  
                                        <FormHelperText color={'var(--errorBase)'} fontSize={'12px'}>{ isSignup?.confirmPassword !== isSignup?.password  ? "" : "" }</FormHelperText>
                                    }
                            </FormControl>
                        </Flex>
                        <Flex 
                        ...
                        <Btn 
                            ...
                            borderRadius={'10px'} textColor={'#FFFFFF'} fontSize={{base:'12px',lg:'14px'}}
                        >
                            Sign Up
                        </Btn>
                        <Btn onClick={HandleGoogleSubmit}
                             bg={'transparent'} my={2}
                            borderColor={'var(--primaryBase)'} display={'flex'} alignItems={'center'}
                            w={"100%"} h={"40px"} border={"1px"} borderRadius={'10px'} textColor={'var(--primaryBase)'}
                            fontSize={{base:'12px',lg:'14px'}} gap={'4px'}
                        >
